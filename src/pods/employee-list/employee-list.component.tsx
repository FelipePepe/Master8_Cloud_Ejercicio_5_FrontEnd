...
interface Props {
  employees: Employee[];
  onEditEmployee: (id: string, idMember: string) => void;
}
  ...
  return (
    <TableContainer>
      <Table>
        ...
        <TableBody>
          {employees.map(row => (
            <TableRow key={row.idMember}>
              <TableCell component="th" scope="row">
                <Checkbox checked={row.active} disabled />
              </TableCell>
              <TableCell align="right">{row.idMember}</TableCell>
              <TableCell align="right">{row.name}</TableCell>
              <TableCell align="right">{row.email}</TableCell>
              <TableCell align="right">{row.lastdate}</TableCell>
              <TableCell align="right">
                <IconButton
                  onClick={() => onEditEmployee(row._id, row.idMember)}
                >
                  <EditIcon />
                </IconButton>
                <IconButton
                  onClick={() => console.log('on Delete: ${row.idMember}')}
                >
                  <DeleteIcon />
                </IconButton>
              </TableCell>
